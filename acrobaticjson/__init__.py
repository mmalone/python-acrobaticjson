...
class Root(object):
    ...
    def for_value(cls, value):
        ...
        """
        self = cls([])
        id = self._acrobatic_value(value)
        assert id == 0
        self.modified = False
        return self
    ...
    def _acrobatic_value(self, value):
        ...
        if isinstance(value, (dict, ObjectWrapper)):
            value_to_store = {}
            for key in value:
                value_to_store[key] = self._acrobatic_value(value[key])
        elif isinstance(value, (list, ArrayWrapper)):
            value_to_store = map(lambda v : self._acrobatic_value(v), value)
        else:
